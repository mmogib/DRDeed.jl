function gtdrdeed(customers::Int = 5, generators::Int = 6, periods::Int = 24)
  ...
  return function solvegtdrdeed(
    w::Vector{Float64} = ones(4);
    solver = ipopt,
    gtdata::GTDeedData = gtdata,
  )
    ...
    if (has_values(model))
      ...
      return SuccessResult(gtdata, solution)
    else
      return FailResult(gtdata, termination_status(model))
    end
  end
end

function drdeed(customers::Int = 5, generators::Int = 6, periods::Int = 24)
  data = getDRDeedData(customers, generators, periods)
  ipopt = optimizer_with_attributes(
    Ipopt.Optimizer,
    MOI.Silent() => true,
    "sb" => "yes",
  ...
  return function solvedrdeed(w::Vector{Float64} = ones(3); solver = ipopt, data::DeedData = data)
    model = Model(solver)
    @variable(model, q[1:generators, 1:periods] >= 0)
    @variable(model, χ[1:customers, 1:periods] >= 0)
    @variable(model, ω[1:customers, 1:periods] >= 0)
...
function deed(generators::Int = 6, periods::Int = 24)
  data = getDRDeedData(5, generators, periods)
  ipopt = optimizer_with_attributes(
    Ipopt.Optimizer,
    MOI.Silent() => true,
    "sb" => "yes",
  ...
  return function solvedrdeed(w::Vector{Float64} = ones(2); solver = ipopt, data::DeedData = data)
    model = Model(solver)
    @variable(model, q[1:generators, 1:periods] >= 0)
